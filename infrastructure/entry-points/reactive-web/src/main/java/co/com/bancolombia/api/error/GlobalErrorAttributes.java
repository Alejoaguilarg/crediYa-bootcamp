    ...
    public Map<String, Object> getErrorAttributes(ServerRequest request,
        ...
        if (error instanceof ServerWebInputException || error instanceof DecodingException) {
            return Map.of(
                    STATUS, 400,
                    ERROR, "Bad Request",
                    MESSAGE, String.format("The field %s is invalid", request.path()),
                    "path", path,
        ...
        if (error instanceof ValidationRuleException exception) {
            ExceptionMapping mapping = ExceptionMapping.from(exception);
            return Map.of(
                    STATUS, mapping.getStatus(),
                    ERROR, mapping.getReason(),
